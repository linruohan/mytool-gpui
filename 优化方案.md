...
| ✅ | P1-1: 改进状态封装 | 2026-05-04 | 添加 ItemInfoMessage 消息传递 |
| ✅ | P1-2: 抽象查询模式 | 2026-05-04 | 添加 query_items 通用方法 |
| ✅ | P1-3: 消除服务层重复模式 | 2026-05-04 | 添加 deprecated 标记，统一 API |
| ✅ | P1-4: 动作层错误处理抽象 | 2026-05-04 | 添加 ActionExecutor 统一错误处理 |
...
| ✅ | P2-3: 实体模型字段拆分 | 2026-05-04 | 添加字段分组辅助方法 |
| ✅ | P2-4: EventBus 泛型事件系统 | 2026-05-04 | 添加 GenericEventBus 泛型支持 |
...
**实施内容**:
1. 添加 `ItemBaseInfo` 结构体（id, content, description）
2. 添加 `ItemTimeInfo` 结构体（added_at, completed_at, updated_at, due_date）
3. 添加 `ItemHierarchyInfo` 结构体（section_id, project_id, parent_id）
4. 添加 `ItemStatusInfo` 结构体（checked, is_deleted, collapsed, pinned）
5. 添加 `ItemOrderInfo` 结构体（priority, child_order, day_order）
6. 为 `ItemModel` 添加分组访问方法

**代码变更**:
- 文件: `crates/todos/src/entity/items.rs`
- 新增: ~100 行（5 个分组结构体 + 5 个访问方法）
- 导出: 更新 `entity/mod.rs`
...
#### ✅ P2-4: EventBus 泛型事件系统

**实施内容**:
1. 添加 `EventTrait` trait 定义泛型事件约束
2. 添加 `GenericEventBus` 支持类型安全的事件发布/订阅
3. 添加 `TypedSubscription<T>` 泛型订阅类型
4. 为 `Event` 枚举实现 `EventTrait`
5. 添加 `ItemEvent` 和 `ProjectEvent` 示例事件类型

**代码变更**:
- 文件: `crates/todos/src/services/event_bus.rs`
- 新增: ~80 行（泛型事件系统）
- 保持向后兼容：原有 Event 枚举和 EventBus 不变
...
**实施内容**:
1. 添加 `ItemInfoMessage` 消息枚举定义
2. 为 `ItemInfoState` 添加 `handle_message` 方法
3. 添加 `receive_item_update` 内部方法
4. 支持消息类型：UpdateItem, RefreshLabels, SaveAllChanges, SetCompleted, SetPinned

**代码变更**:
- 文件: `crates/mytool/src/ui/components/item_info.rs`
- 新增: ~40 行（消息枚举 + 处理方法）
...
**实施内容**:
1. 创建 `ActionExecutor` 结构体统一错误处理
2. 实现 `execute` 方法（带验证）
3. 实现 `execute_without_validation` 方法
4. 实现 `execute_delete` 方法（删除操作）
5. 添加便捷验证器函数

**代码变更**:
- 文件: `crates/mytool/src/core/actions/executor.rs`（新建）
- 新增: ~220 行
- 导出: 更新 `actions/mod.rs`
