...
impl Window {
    pub fn init(cx: &mut gpui::Window) {
        let view = cx.new_context(|cx| {
    ...
    pub fn is_open(cx: &mut Context<Self>) -> bool {
        cx.read_global::<Self, _>(|w, _| !w.hidden)
    }
    ...
    pub fn toggle(cx: &mut Context<Self>) {
        cx.update_global::<Self, _>(|this, cx| {
            if this.hidden {
                cx.active_window();
                this.hidden = false;
    ...
    pub fn close(cx: &mut Context<Self>) {
        cx.update_global::<Self, _>(|this, cx| {
            this.hidden = true;
            cx.hide();
        });
