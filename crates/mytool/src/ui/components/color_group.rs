...
impl ColorGroup {
    ...
    fn render_colors(&self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        v_flex().gap(VisualHierarchy::spacing(3.0)).items_center().child(
            v_flex().gap(VisualHierarchy::spacing(1.0)).children(color_palettes().iter().map(
                |sub_colors| {
                    h_flex().gap(VisualHierarchy::spacing(1.0)).children(sub_colors.iter().map(
                        |color| {
                            let color = *color;
                            div()
                                ...
                                .active(|this| {
                                    this.border_color(color.lighten(0.5))
                                        .bg(color.darken(0.2))
                                        .border_2()
                                })
