...
async fn refresh_items(cx: &mut AsyncApp, db: DatabaseConnection) {
    ...
    cx.update_global::<ScheduledItemState, _>(|state, _| {
        state.set_items(items);
    })
    .ok();
}
// 添加item
pub fn add_scheduled_item(item: Rc<ItemModel>, cx: &mut App) {
...
// 修改item
pub fn update_scheduled_item(item: Rc<ItemModel>, cx: &mut App) {
    let conn = cx.global::<DBState>().conn.clone();
    cx.spawn(async move |cx| {
        let db = conn.lock().await;
...
// 删除item
pub fn delete_scheduled_item(item: Rc<ItemModel>, cx: &mut App) {
    let conn = cx.global::<DBState>().conn.clone();
    cx.spawn(async move |cx| {
        let db = conn.lock().await;
