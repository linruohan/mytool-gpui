...
use crate::todo_state::{DBState, PinnedItemState};

// 刷新items
async fn refresh_items(cx: &mut AsyncApp, db: DatabaseConnection) {
    let items = crate::service::get_items_pinned(db).await;
    cx.update_global::<PinnedItemState, _>(|state, _| {
        state.set_items(items);
    })
    .ok();
}
// 添加item
pub fn add_pinned_item(item: Rc<ItemModel>, cx: &mut App) {
...
// 修改item
pub fn update_pinned_item(item: Rc<ItemModel>, cx: &mut App) {
    let conn = cx.global::<DBState>().conn.clone();
    cx.spawn(async move |cx| {
        let db = conn.lock().await;
...
// 删除item
pub fn delete_pinned_item(item: Rc<ItemModel>, cx: &mut App) {
    let conn = cx.global::<DBState>().conn.clone();
    cx.spawn(async move |cx| {
        let db = conn.lock().await;
