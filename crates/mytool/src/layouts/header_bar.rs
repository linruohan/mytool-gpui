...
impl crate::Mytool for HeaderBar {
    fn title() -> &'static str {
        "Alert"
    }
...
impl Render for HeaderBar {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        v_flex().gap_4().child(
            ...
            ).banner().on_close(cx.listener(|this, _, _, cx| {
                this.banner_visible = !this.banner_visible;
                cx.notify();
            })).visible(self.banner_visible).with_size(self.size).icon(IconName::Bell),
        ).child(
            ButtonGroup::new("toggle-size").outline().compact().child(
            ...
            section("Info").w_2_3().child(
                Alert::info("info1", "This is an info alert.").with_size(self.size).title("Info message").on_close(cx.listener(|_, _, _, _| {
            ...
            section("Success with Title").w_2_3().child(
                ...
                ).with_size(self.size).title("Submit Successful"),
            ),
        ).child(
            ...
            section("Error").w_2_3().child(
                ...
                ).with_size(self.size).title("Error!"),
            ),
        ).child(
            section("Custom Icon").w_2_3().child(
                ...
                ).title("Custom Icon").with_size(self.size).icon(IconName::Bell),
            ),
        )
    }
}
